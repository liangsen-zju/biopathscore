lpc <- function(X,h, t0=mean(h),  x0=1,   way = "two", scaled=TRUE,  weights=1, pen=2,
  ...
  # scaled: control the bandwidth h, control: PCA matrix scaled
  # mult??Number of starting points manually enforced to be mult
  # way: "two"
  
  #browser()
  iter    <-  control$iter      # 100
  boundary <- control$boundary  # 0.005
  convergence.at <- control$convergence.at      #1e-0.5
  thresh  <- control$pruning.thresh             # 0
  rho0    <- control$rho0                       # 0.4
  cross   <- control$cross                      # FALSE
  mult    <- control$mult                       # 1
  
  Xi      <- as.matrix(X)                       
  N       <- dim(Xi)[1]                         # 1208, #samples
  d       <- dim(Xi)[2]                         # 10,   #PCAs
  
  # weights = ranks, all normal is a value, all tumor is a other value
  ...
  s1       <- apply(Xi, 2, function(dat){ diff(range(dat))})  # range of the PCAs
  ...
  # x0 starting points: x0 = null/0/1/vector(d)
  if (length(x0)==1 && x0==1){
    
    # x0 = 1: use mean-shift to get the local max mass points (by random)
    n  <- sample(N,1);
    x0 <- matrix(ms.rep(Xi, Xi[n,],ms.h, plotms=0)$final, nrow=1)
    
  }  else if (length(x0)==1 && x0==0 ){
    
    # x0 = 0: use the mean-shift cluster
    if (N <= ms.sub) {
      sub<- 1:ms.sub
    }  else {    
    ...
    # use pre-selection starting points
    if (is.null(x0)){
  ...
  for (j in 1:mult){
    ...
    if (depth >1 && l>=1 ){							  # constructs branches of depth 2.
      ...
      for (s in 1:l){
        ...
        center.x   <- cov.wt(Xi, wt= kernd(Xi,x,h)*weights) #, method="ML")           # computes local covariance and mean at x
    ...
    if (depth >2 && k>=1 ){					# constructs branches of depth 3
      for (s in 1:k){
        ...
        center.x  <- cov.wt(Xi, wt= kernd(Xi,x,h)*weights) #, method="ML")
